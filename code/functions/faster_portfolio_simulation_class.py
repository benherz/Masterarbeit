...
class PortfolioSimulation_fast:
  ...
  ##### Function to simulate a stock sale
    def sell(self, cik, price, date):
        """
        Execute a sell transaction for all held shares of the specified stock, if any are held.

        Checks if the stock position exists and is greater than zero before selling.
        Updates cash balance, reduces the stock position by held shares, records the transaction,
    ...
    def simulate_trading(self):
        ...
        Returns:
            None
        """
        # Trading simulation iterates over the unique months in the recommendations
        unique_dates = self.recommendations['date'].sort_values(ascending = True).unique()

        # Skip trading for last month, as no returns can be computed
                ...
                if qty > 0:
                    try: 
                       ...
                       # sell_counter += 1
                        ...
                        self.skipped_transactions.append(("Sell signal", cik, date, str(e)))
                        self.no_skipped_sells += 1
                        self.no_skipped_transactions += 1
    ...
    def get_portfolio_value(self, date=None):
        # If no date is provided, last date in recommendations is used
        if date is None:
            date = pd.Period(self.recommendations['date'].max(), freq='M')
        # Convert input date to Period (monthly)
        if not isinstance(date, pd.Period):
            date = pd.to_datetime(date).to_period('M')
